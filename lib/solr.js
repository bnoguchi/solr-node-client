...
Client.prototype.update = function(data,callback){
   ...
   var fullPath = [this.options.path, this.options.core, 'update'];
   var fullPath = this.options.path +
                  (this.options.core ? '/' + this.options.core : '') +
                  '/update?wt=json';
   if (!data.commit) {
     if (this.autoCommit) {
       fullPath += '&commit=true';
     }
     if (this.softCommit) {
       fullPath += '&softCommit=true';
     }
   }

   this.options.fullPath = fullPath;
...
var updateRequest = function(params,callback){
   ...
   var callbackResponse = function(res){
      ...
      res.on('end',function(){
         ...
         } else if (buffer.slice(0, 5) === '<?xml') {
            err = new Error();
            err.message = buffer.toString();
