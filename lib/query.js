...
/**
 * Sets the operator to the request to AND or OR (default is OR).
 *
 * @param {String} andOr can be AND or OR
 *
 * @return {Query}
 * @api public
 */
Query.prototype.op = function(andOr) {
  var parameters = this.parameters;
  for (var i = 0, l = parameters.length; i < l; i++) {
    var param = parameters[i];
    if (param.slice(0, 2) !== 'q=') continue;
    parameters[i] = param.slice(0, 2) + encodeURIComponent('{!lucene q.op=' + andOr + '}') + param.slice(2);
  }
  this.booleanOp = andOr;
  return this;
}
...
Query.prototype.q = function(q){
   var self = this;
   var parameter ='q=';
   if (this.booleanOp) parameter += encodeURIComponent('{!lucene q.op=' + this.booleanOp + '}');
   if ( typeof(q) === 'string' ){
...
Query.prototype.facet = function(options){
   ...
   }else if (this.parameters.indexOf('facet=true') === -1) {
      this.parameters.push('facet=true');
   }
   if(options.query){
      this.parameters.push('facet.query=' + (options.ex || '') + encodeURIComponent(options.query))
   }
   if(options.field){
   ...
   var fieldSpecificFacetOptions = ['limit', 'mincount', 'offset', 'missing', 'method', 'sort', 'prefix'];
   for(var k in options) {
     if(k.slice(0,2) !== 'f.') continue;
     for(var i = 0, l = fieldSpecificFacetOptions.length; i < l; i++) {
       if(~k.indexOf(fieldSpecificFacetOptions[i])) {
         this.parameters.push(k + '=' + options[k]);
         break;
       }
     }
   }
